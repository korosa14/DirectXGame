	...
	IDxcIncludeHandler* includeHandler)
{
	...
	HRESULT hr = dxcUtile->LoadFile(filePath.c_str(), nullptr, & shaderSource);
	//読めなかったら止める
	assert(SUCCEEDED(hr));
	//読み込んだファイル内容設定
	...
	assert(SUCCEEDED(hr));
	//成功したログを出す
	Log(ConvertString(std::format(L"Copile Succeeded,path:{},profile:{}\n", filePath, profile)));
	//ソースを解放
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	rtvDescriptorHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;//レンダータ-ゲットビュー用
	...
	if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debugController)))) {
		...
		//GPU側でもチェック
		debugController->SetEnableGPUBaseValidation(TRUE);
	}
#endif
	...
	if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&infoQueue)))) {
		//ヤバいエラー時に止まる
		infoQueue->SetBreakOnSeverity(D3D12_MASSAGE_SEVERITY_CORRUPTION, true);
		//エラー時に止まる
		...
		D3D12_MESSAGE_ID denyIds[] = {
			D3D12_MESSAGE_ID_RESOURCE_BARRIER_MISMATND_LIST_TYPE
		};
		//抑制レベル
		...
		filter.DenyList.NumSeveritise = _countof(severities);
	...
	vertexResourceDecc.Width = sizeof(Vector4) * 3;
	...
	hr = device->CreateCommittedResource(&uploadHeapProperties, D3D12_HEAP_FLAG_NONE,
		&vertexResourceDecc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
	...
	commandList->RSSetScissorRects(1, &scissorRect);//Scirssosを設定
	//RootSignatureを設定
